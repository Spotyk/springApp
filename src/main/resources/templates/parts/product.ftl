...
<#macro product path>
    <form method="POST" action="${path}" enctype="multipart/form-data">
        ...
        <div class="input-group mb-3">
            <div class="input-group-prepend">
                <span class="input-group-text" id="product-addon"><@spring.message "productName"/> <@spring.message "ru"/>:</span>
            </div>
            <input type="text" class="form-control" name="productNameRu" value="${(currentProduct.productNameRu)!}">
        </div>
                <div class="input-group mb-3">
            <div class="input-group-prepend">
                <span class="input-group-text" id="product-addon"><@spring.message "productName"/> <@spring.message "en"/>:</span>
            </div>
  ...
  <div class="mb-3">
    ...
    <textarea name="descriptionRu" class="form-control" id="validationTextarea" required>${(currentProduct.descriptionRu)!}</textarea>
    ...
    <textarea name="descriptionEn" class="form-control" id="validationTextarea"  required>${(currentProduct.descriptionEn)!}</textarea>
    <div class="invalid-feedback">
        Please enter a message in the textarea.
    </div>
  </div>
    <div class="input-group mb-3">
        ...
        <input class="btn btn-primary" type="submit" value="<@spring.message "update"/> <@spring.message "product"/>">
        <#else>
        <input class="btn btn-primary" type="submit" value="<@spring.message "create"/> <@spring.message "product"/>">
        </#if>
    </form>
</#macro>