    ...
    <form method="POST" action="${path}">
        ...
        <div class="input-group mb-3">
            ...
            <#else>
                <span class="input-group-text"
                      id="category-addon"><@spring.message "categoryName"/> <@spring.message "ru"/> :</span>
                <input type="text" class="form-control" name="categoryNameRu">
                <span class="input-group-text"
                      id="category-addon"><@spring.message "categoryName"/> <@spring.message "en"/>:</span>
        ...
        <#if path == "/updateCategory">
            <input class="btn btn-primary" type="submit"
                   value="<@spring.message "update"/> <@spring.message "category"/>">
        <#else>
            <input class="btn btn-primary" type="submit"
                   value="<@spring.message "create"/> <@spring.message "category"/>">
        </#if>
    </form>
</#macro>